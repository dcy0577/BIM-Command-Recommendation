...
class TransformerBlock(BlockBase):
    ...
    def __init__(
        ...
        self.item_embedding_table = embedding_layer
        if self.item_embedding_table is not None:
            hidden_size = self.transformer.config.d_model
            self.start_embed = torch.nn.Parameter(torch.randn(hidden_size))
    ...
    def forward(self, inputs_embeds, **kwargs):
        ...
        # consturct the decoder input embeds from the label(target sequence)
        # designed for the MLM task
        if self.item_embedding_table is not None:
            item_ids = self.masking.decoder_target_sequence
            batch_size, seq_len = item_ids.size()
            item_embeds = self.item_embedding_table(item_ids)
            # mask the last item to predict during the evaluation
            if self.masking.eval_on_last_item_seq_only and not self.training:
                # replaces the items in inputs where mask_schema is True. The argument sequence is different than in CLM
                label_embeds = torch.where(
                    self.masking.mask_schema.unsqueeze(-1).bool(),
                    self.masking.masked_item_embedding.to(item_embeds.dtype),
                    item_embeds,
                )
            else:
                label_embeds = item_embeds
            decoder_inputs_embeds = torch.zeros_like(label_embeds)
            decoder_inputs_embeds[:, 0, :] = self.start_embed
            decoder_inputs_embeds[:, 1:, :] = label_embeds[:, :-1, :]

            transformer_kwargs["decoder_inputs_embeds"] = decoder_inputs_embeds
        ...
        if self.transformer.config.model_type == "mixtral":
            # we need expert outputs for computing the load balancing loss
            outputs = model_outputs
        else:
            # this is only for the hidden states
