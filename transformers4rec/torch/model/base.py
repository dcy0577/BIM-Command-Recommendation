...
from transformers.modeling_utils import SequenceSummary, PreTrainedModel, PretrainedConfig
...
class Model(torch.nn.Module, LossMixin, MetricsMixin):
    def __init__(
        ...
        *head: Head,
        ...
        self.top_k = top_k
        self.config = PretrainedConfig(model_type="t4rec_model") # dummy config for if else condition in peft_model.py
        self.is_loaded_in_4bit = True # dummy config for if else condition in peft/tuners/tuners_utils.py
        self.is_loaded_in_8bit = False if self.is_loaded_in_4bit else True # dummy config for if else condition in peft/tuners/tuners_utils.py
    ...
    def forward(self, inputs: TabularData, targets=None, training=False, testing=False, **kwargs):
        ...
        if training or testing:
            ...
            labels = {}
            predictions = {}
            # so I can use two head transformers here with 2 TabularFeaturesModule and corresponding transformer blocks
    ...
    # will this be used? for llamamodel not! Just for checking purposes
